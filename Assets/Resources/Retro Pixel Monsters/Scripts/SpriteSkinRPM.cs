...
public class SpriteSkinRPM : MonoBehaviour {
	...
	private Image parentRenderer; //The object's sprite renderer.
	...
	void Start () {
		if (parentRenderer == null) {
			parentRenderer = GetComponent<Image>();
		}
		if (newSprite != null) {
	...
	void LateUpdate () {
		...
		if (newSprite != null && parentRenderer != null && newSpritesheet.Length > 0){
			...
			string test = Regex.Replace(currentFrameName, "[^0-9]", "");
			Debug.Log(test);
			int.TryParse(test, out frameIndex);
